	...
	request(api_uri, { json: true }, (err, res, body) => {
		if (err) {
			...
			if(body.examples.length == 0){
				response.error = true;
				response.error_message = `1.You might have mis-spelled the word \n2. Examples for this word does not exist\n3. We are unable to find the word Examples`;
				return callback(response);
			}else{
...
Wordnick.prototype.randomWord = function(callback){
	let endpoint = "words.json/randomWord";
	let api_uri = this.api + endpoint + "?api_key="+this.api_key;
	let response = {
		"error" : false,
		"error_message" : "",
